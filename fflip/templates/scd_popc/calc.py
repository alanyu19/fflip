...
#from fflip.chm.dppc import *

parser = argparse.ArgumentParser()

#parser.add_argument('--pdb', type = str, default = 'bi_72_dppc_c36.pdb', help = "pdb file")
...
scd_evaluators = []
for i in range(3, sn2):
    if i not in sn2_ch1:
        scd_evaluators.append(ef.order_parameter(topology, 'C2{}'.format(i), ['H{}R'.format(i), 'H{}S'.format(i)]))
    else:
        scd_evaluators.append(ef.order_parameter(topology, 'C2{}'.format(i), 'H{}1'.format(i)))

for i in range(3, sn3):
    if i not in sn3_ch1:
        scd_evaluators.append(ef.order_parameter(topology, 'C3{}'.format(i), ['H{}X'.format(i), 'H{}Y'.format(i)]))
    else:
        scd_evaluators.append(ef.order_parameter(topology, 'C3{}'.format(i), 'H{}1'.format(i)))
...
scd_evaluators.append(ef.order_parameter(topology, 'C32', ['H3X', 'H3Y']))

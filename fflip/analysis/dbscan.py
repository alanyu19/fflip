...
def res_info(x_pt, y_pt, block_data):
    for entry in block_data:
        if str(x_pt) in entry and str(y_pt) in entry:
            resn = entry[0]
            resid = entry[1]
            nim = entry[2]
...
def function(
    ...
    for fr in range(nframe):
        ...
        if info2 is not None:
            info2s.append(info2)
    with open(num_file_template.format(index_trj), 'w') as f:
...
def func2(Data, left, right, Dcut, index_trj, frame, nlip):
    ...
    info1, info2 = run_dbscan(data, Data, Idyn, Dcut, left, right, Fr)
    return info1, info2


def run_dbscan(data, Data, Idyn, Dcut, left, right, frame, fnum, ftot):
    line = 0
    # Compute DBSCAN
    db = DBSCAN(eps=Dcut, min_samples=3).fit(data)
            ...
            for q in range(0, len(comp_1c), 1):
                ...
                if Occ > 1:
                    ...
                    raise Exception(
                        ...
                        "Resid", Lresid_1c[i], "is duplicated", Occ,

...
import os
import shutil
from fflip.core import filesys
...
def replace_placeholders(filename, para_names=None, para_values=None,
                         para_dict=None, characters=7, digits=4):
            ...
            for i in range(len(para_names)):
                ...
                if part1 in nl:
                    if part1 + "?" in nl:  # format: ?_name_?
                        ...
                        nl = nl.replace(
                            "{}{}_?".format(part1, value),
                            "{0:.{1}f}".format(para_values[i], digits)
                        )
        print(nl.replace("\n", ""))  # the comma supresses adding another
        # newline statement
